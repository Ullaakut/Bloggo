...
* I know that storing the secret in the sources isn't a good practice, so normally I would make a deployment script that requires a password to decrypt the secret and store it in the environment, but here since I don't want this to be too complicated, I'd like to make it simple to test, even if it means it's less secure.
